...
for i in {00000..00831}; do
  FILE_PATH="${OUTPUT_DIR}/${i}.tar"
  if [ -f "$FILE_PATH" ]; then
    FILE_SIZE=$(stat -c%s "$FILE_PATH")
    if [ "$FILE_SIZE" -ge 104857600 ]; then
      echo "File ${i}.tar already exists and is larger than or equal to 100MB. Skipping download."
      continue
    fi
  fi

  wget -O "$FILE_PATH" -c "https://cloud.mlcommons.org/index.php/s/training_stable_diffusion/download?path=/datasets/laion-400m/moments-webdataset-filtered&files=${i}.tar"
done
