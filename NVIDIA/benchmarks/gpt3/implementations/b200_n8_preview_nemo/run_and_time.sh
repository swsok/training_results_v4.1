...
if [[ "$RANK" -eq 0 ]]; then
  echo "Run vars: id ${SLURM_JOB_ID:-'not invoked by slurm'} gpus $LOCAL_WORLD_SIZE mparams $MULTI_NODE"
  START=$(date +%s)
...
if [ ${NVTX_FLAG} -gt 0 ]; then
 NSYSCMD=" nsys profile --sample=none --cpuctxsw=none  --trace=cuda,nvtx --cuda-graph-trace=node --force-overwrite true  --output /results/llm_${DGXNNODES}_r${SLURM_PROCID}.nsys-rep "
